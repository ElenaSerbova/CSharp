...
namespace FunWithLinqExpressions
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //AlphabetizeProductNames1(itemsInStock);
            //Console.WriteLine();
        ...
        static void ListProductNames2(ProductInfo[] products)
        {
            // Now get only the names of the products.
            Console.WriteLine("Only product names:");

            //var names = from p in products
            //            select new {Name = p.Name, Quantity = p.NumberInStock};

            var names = products.Select(p => new { Name = p.Name, Quantity = p.NumberInStock });

            foreach (var a in names)
            {
                Console.WriteLine("Name: {0}, Quantity: {1}", a.Name, a.Quantity);
            }
        }
        #endregion
        ...
        static void AlphabetizeProductNames1(ProductInfo[] products)
        {
            // Get names of products, alphabetized.
            var subset = from p in products
                         orderby p.NumberInStock, p.Name 
                         select p;

            var subset1 = products
                            .OrderBy(p => p.NumberInStock)
                            .ThenBy(p => p.Name)
                            .Select(p => p);
        ...
        static void GetCountFromQuery()
        {
            ...
            int numb1 = currentVideoGames
                            .Where(g => g.Length > 6)
                            .Count();

            int numb2 = currentVideoGames.Count(g => g.Length > 6);
        ...
        static void DisplayConcat()
        {
            ...
            foreach (string s in carConcat)
                Console.WriteLine(s); // "Yugo", "Aztec", "BMW", "BMW", "Saab", "Aztec" 
        }
        ...
        static void SelectWithIndex(ProductInfo[] itemsInStock)
        {
            var res = itemsInStock
                .Where((p, i) => (i & 1) == 0)
